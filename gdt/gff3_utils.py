...
def load_gff3_old(filename,
              ...
              query_string=''):
      if query_string:
            return (pd.read_csv(filename, sep=sep, comment=comment, header=header, names=names, usecols=usecols)
                    .query(query_string))
      
      return (pd.read_csv(filename, sep=sep, comment=comment, header=header, names=names, usecols=usecols))

def filter_orfs_old(gff3_df, orfs_strings=['Name=ORF', 'Name=orf']):
      return (gff3_df[~gff3_df['attributes'].str.contains('|'.join(orfs_strings))]
              .sort_values(by=['start', 'end'], ascending=[True, False], ignore_index=True)) # type: ignore
...
def load_gff3(filename,
      ...
      if query_string:
            return (pd.read_csv(filename, sep=sep, comment=comment, header=header, names=names, usecols=usecols)
                    .query(query_string).sort_values(by=['start', 'end'], ascending=[True, False], ignore_index=True)) # type: ignore

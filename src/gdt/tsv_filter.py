    ...
    try:
       ...
       # Get dbxref info
        ...
        if not all(in_gene_dict_mask):
            status = 'missing_gene_dict_with_dbxref' if all(dbxref_mask) else 'missing_dbxref'
        ...
        return {
            ...
            'genes_not_in_dict': [g for g, in_dict in zip(gene_ids, in_gene_dict_mask) if not in_dict],
            'genes_in_dict': [g for g, in_dict in zip(gene_ids, in_gene_dict_mask) if in_dict]
        }
    except Exception as e:
       return {'AN': AN, 'status': 'error', 'error': str(e)}

def filter_whole_tsv(logger: logging.Logger, tsv_path: Path, gdt_path: Optional[Path] = None, keep_orfs=False, workers=0, AN_column='AN'):
    ...
    logger.trace(f'filter_whole_tsv called | tsv_path: {tsv_path} | gdt_path: {gdt_path} | w: {workers} | keep_orfs: {keep_orfs}')
    ...
    for future in futures:
        ...
        logger.trace(f"\tgenes: {result['gene_count']} | have dbxref: {result['dbxref_count']} | "
        ...
        if result['status'] == 'missing_gene_dict_with_dbxref':
            ...
            logger.trace(f'\t{AN} is missing genes in gene_dict and is also missing dbxref')
